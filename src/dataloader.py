...
class FMVIDataset(Dataset):
    def __init__(self,root,feat_dim,eigen_num,device_id = 0,batch_size = 1,root_cons = None,tss = False,case = None,Tool = None,use_dino=False):
        super().__init__()
        self.root = root
        ...
        self.samples = self.samples[case*2+ device_id * 2 : case*2+(device_id + 1) * 2] if not Tool else self.samples
        ...
        self.case = case
        self.cons_samples = self.cons_samples[case*2+ device_id*2:case*2+(device_id + 1) * 2] if not Tool else self.cons_samples
    ...
    def cons_block(self,i,j,transiton_matrix,omega,eigen_num = None,device = None):
      ...
      #  cur_idx = batch_idx * self.batch_size + i
        if i==j:
    ...
    def compute_T(self,feat_proj_x,feat_proj_y,T):
    
        feat_proj_x_ = feat_proj_x.detach()
        feat_proj_y_ = feat_proj_y.detach()
        def objective(T):
            transfer_feat = torch.matmul(T, feat_proj_x_)
            objective = torch.norm(transfer_feat - feat_proj_y_, p='fro')
    ...
    def __getitem__(self, index):
        ...
        for _ in range(self.batch_size):
            ...
            if batch_index not in self.eval_cache:
                ...
                path1, path2 = self.cons_samples[idx1], self.cons_samples[idx2]
                path1_loss,path2_loss = self.samples[idx1], self.samples[idx2]
                ...
                feat_x_loss = self.loader(path1_loss).permute(1,2,0)
                feat_y_loss = self.loader(path2_loss).permute(1,2,0)
                fmap1 = Functional_Map(feat_x, feat_y, eigen_num=200, dim=self.feat_dim, use_built_in=False, use_dino=self.use_dino,device_id = self.device_id)
                ...
                self.eval_cache[batch_index] = {'feat_x': feat_x, 'eval_x': eval_x, 'evec_x': evec_x, 'feat_y': feat_y, 'eval_y': eval_y, 'evec_y': evec_y, 'fmap1': fmap1, 'mask1': mask1, 'mask2': mask2, 'feat_x_loss':feat_x_loss, 'feat_y_loss':feat_y_loss}
        ...
        batch = {
            ...
            'feat_x_loss':torch.stack([item['feat_x_loss'] for item in batch_data]),
            'feat_y_loss':torch.stack([item['feat_y_loss'] for item in batch_data]),
        }
